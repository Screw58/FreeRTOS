...
#define task1_port	GPIOC
#define task2_port	GPIOC
/* USER CODE END PTD */
...
BaseType_t status=0;
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  status = xTaskCreate(task1_func, "Task-1", 200, NULL, 1, &task1_handle);
  configASSERT(status == pdPASS);
  status = xTaskCreate(task2_func, "Task-2", 200, NULL, 2, &task2_handle);
  configASSERT(status == pdPASS);
...
void task1_func(void * param)
{
	while(1)
	{
		HAL_GPIO_TogglePin(task1_port, task1_pin);
		HAL_Delay(1000);
		switch_priority();
	}
...
void task2_func(void * param)
{
	while(1)
	{
		HAL_GPIO_TogglePin(task2_port, task2_pin);
		HAL_Delay(100);
		switch_priority();
	}
...
void switch_priority(void)
{
	TaskHandle_t t1, t2, current;
	UBaseType_t p1, p2;

	uint8_t flag_priority =0;
